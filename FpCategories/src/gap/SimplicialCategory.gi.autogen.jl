...
@InstallGlobalFunction( SimplicialCategoryTruncatedInDegree,
  ...
  function ( CAP_NAMED_ARGUMENTS, n )
    ...
    if (n == 0)
        ...
        F = PathCategory( FinQuiver( "Delta(C0)[]" ); range_of_HomStructure = range_of_HomStructure, FinalizeCategory = FinalizeCategory );
    ...
    elseif (n == 1)
        ...
        rels = List( [ [ "s*id", "id_C0" ],
                        [ "t*id", "id_C0" ] ],
                  pair -> PairGAP( CallFuncListAtRuntime( /, [ pair[1], F ] ), CallFuncListAtRuntime( /, [ pair[2], F ] ) ) );
        
        Delta = CallFuncListAtRuntime(
                    () -> QuotientCategory( F, rels; range_of_HomStructure = range_of_HomStructure, FinalizeCategory = FinalizeCategory ),
                    [] );

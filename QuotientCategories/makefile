...
uninstall:
	julia -e 'using Pkg; Pkg.rm("QuotientCategories");'

test:
	julia -e 'using Pkg; Pkg.test("QuotientCategories");'
...
git-commit:
	...
	echo -e "\033[32mCommitting changes in \"QuotientCategories\"\033[0m"; \
	git add .; \
	git commit \
		...
		echo -e "\033[33mNo changes in \"QuotientCategories\" to commit or no version bump detected.\033[0m"; \
	fi

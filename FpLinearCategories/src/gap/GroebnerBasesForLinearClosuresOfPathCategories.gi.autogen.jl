...
@InstallMethod( ReductionOfMorphism,
  ...
  function ( kC, f, G )
    ...
    while @not IsZeroForMorphisms( kC, f )
        ...
        if (i != fail)
            ...
            u_i = MorphismConstructor( C,
                            ...
                            PairGAP( p - 1, leading_morph_f_datum[2][(1):(p - 1)] ),
                            ObjectDatum( kC, Source( G[i] ) ) );
            
            u_i = MorphismConstructor( kC, Source( f ), PairGAP( [ one ], [ u_i ] ), Source( G[i] ) );
            
            v_i = MorphismConstructor( C,
                            ObjectDatum( kC, Target( G[i] ) ),
                            PairGAP( leading_morph_f_datum[1] - p - leading_morphs_G_datum_i[1] + 1,
                                  leading_morph_f_datum[2][ (p + First( leading_morphs_G_datum_i )):(First( leading_morph_f_datum )) ] ),
                            ObjectDatum( kC, Target( f ) ) );
            
            v_i = MorphismConstructor( kC, Target( G[i] ), PairGAP( [ one ], [ v_i ] ), Target( f ) );
            ...
            leading_term_f = MorphismConstructor( kC,
                                    Source( f ),
                                    PairGAP( [ leading_coeff_f ], [ leading_morph_f ] ),
                                    Target( f ) );
...
@InstallMethod( NewRelation,
  ...
  function ( kC, g, h, overlap_coeffs )
    ...
    l_leading_morph_g = MorphismConstructor( kC, SetOfObjects( kC )[ObjectIndex( Source( overlap_coeffs[1][1] ) )], PairGAP( [ one ], [ overlap_coeffs[1][1] ] ), Source( g ) );
    r_leading_morph_g = MorphismConstructor( kC, Target( g ), PairGAP( [ one ], [ overlap_coeffs[1][2] ] ), SetOfObjects( kC )[ObjectIndex( Target( overlap_coeffs[1][2] ) )] );
    
    l_leading_morph_h = MorphismConstructor( kC, SetOfObjects( kC )[ObjectIndex( Source( overlap_coeffs[2][1] ) )], PairGAP( [ one ], [ overlap_coeffs[2][1] ] ), Source( h ) );
    r_leading_morph_h = MorphismConstructor( kC, Target( h ), PairGAP( [ one ], [ overlap_coeffs[2][2] ] ), SetOfObjects( kC )[ObjectIndex( Target( overlap_coeffs[2][2] ) )] );
...
@InstallMethod( GroebnerBasis,
  ...
  function ( kC, relations )
    ...
    while i <= Length( rels )
        ...
        if (@not IsZeroForMorphisms( kC, rels[i] ))
            
            g = ReductionOfMorphism( kC, rels[i], gb );
            
            if (@not IsZeroForMorphisms( kC, g ))
...
@InstallMethod( ReducedGroebnerBasisWithGivenGroebnerBasis,
  ...
  function ( kC, gb )
    ...
    while i <= Length( reduced_gb )
      ...
      if (IsZeroForMorphisms( kC, r ))
          
          Remove( reduced_gb, i );
          
      elseif (@not IsEqualForMorphisms( kC, r, reduced_gb[i] ))

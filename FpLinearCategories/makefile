...
uninstall:
	julia -e 'using Pkg; Pkg.rm("FpLinearCategories");'

test:
	julia -e 'using Pkg; Pkg.test("FpLinearCategories");'
...
git-commit:
	...
	echo -e "\033[32mCommitting changes in \"FpLinearCategories\"\033[0m"; \
	git add .; \
	git commit \
		...
		echo -e "\033[33mNo changes in \"FpLinearCategories\" to commit or no version bump detected.\033[0m"; \
	fi
